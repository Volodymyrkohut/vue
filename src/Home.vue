<template>
  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <router-link to='/posts'>Про нас</router-link>
    ...
    <div class="container">
      ...
      <template v-if="tickets.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
              v-for="ticket in paginatedTickers"
              :key="ticket.id"
              @click="selectedTicker = ticket"
              :class="{'border-4': selectedTicker === ticket}"
...
import pageLog from "./mixins/pageLog"
...
export default {
  ...
  computed: {
    ...
    getIndexEnd() {
      return this.getIndexStart + 3
    },
    filteredTickets() {
      return this.tickets.filter((item) => item.name.includes(this.filter));
    },
    hasNextPage() {
      return this.filteredTickets.length > this.getIndexEnd
    },
    paginatedTickers() {
      return this.filteredTickets.slice(this.getIndexStart, this.getIndexEnd)
    },
    pageStateOptions() {
      return {
        filter: this.filter,
  ...
  methods: {
    updateTickers(tickerName, price) {
      this.tickets.filter(t => t.name === tickerName).forEach(t => t.price = price)
    },
    ...
    add() {
      ...
      this.tickets = [...this.tickets, item]


      subscribeToTicker(this.ticker.name,(price) => {
        this.updateTickers(this.ticker.name, price)
      });
    },
  ...
  watch: {
    ...
    pageStateOptions(value) {
      history.pushState(null, document.title, `${location.pathname}?filter=${value.filter}&page=${value.page}`)
    },
    filter() {
      this.page = 1
